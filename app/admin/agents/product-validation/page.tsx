...
export const metadata = {
  title: "Product Development & Validation Agent | Ghostlayer Admin",
  description: "Read-only Ghostlayer product validation agent for scan and lead follow-up review.",
};
...
async function fetchTableSummary(check: (typeof productChecks)[number]): Promise<TableSummary> {
...
function buildRecommendations(summaries: TableSummary[]) {
  ...
  if (actions.length === 0) {
    actions.push("No recent product-validation activity found in the last 14 days.");
  }
...
export default async function ProductValidationAgentPage() {
  const summaries = await Promise.all(productChecks.map(fetchTableSummary));
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#05070b] px-6 py-10 text-white sm:px-8 lg:px-10">
      ...
      <section className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="mt-12 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="mt-5 max-w-4xl text-4xl font-black tracking-tight sm:text-5xl">
              Product Development & Validation Agent
            </h1>

            <p className="mt-5 max-w-3xl text-sm leading-7 text-gray-300 sm:text-base">
              Read-only product validation assistant for Ghostlayer. This agent reviews workflow scans, reports, feedback, CTA clicks, and client messages to identify repeated customer pain points, offer gaps, service improvements, and package opportunities for admin review.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.28em] text-emerald-200">
                Product Validation Snapshot
              </p>
              <h2 className="mt-3 text-2xl font-black text-white">
                Recent product-validation activity
              </h2>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-gray-300">
                Showing recent product-validation activity from the last 14 days. This agent can summarize patterns and recommend product or service improvements, but it does not change offers, pricing, or roadmap decisions automatically.
              </p>
            </div>
        ...
        <section className="mt-8 rounded-[2rem] border border-cyan-300/20 bg-cyan-300/10 p-6 shadow-[0_24px_100px_rgba(0,0,0,0.25)] backdrop-blur-xl">
          ...
          <h2 className="mt-3 text-2xl font-black text-white">
            Product opportunities worth reviewing first
          </h2>
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-2">
          {summaries.map((summary) => {
            const check = productChecks.find((item) => item.table === summary.table);
        ...
        <section className="mt-8 rounded-[2rem] border border-yellow-300/20 bg-yellow-300/10 p-6 shadow-[0_24px_100px_rgba(0,0,0,0.25)] backdrop-blur-xl">
          ...
          <h2 className="mt-3 text-2xl font-black text-white">
            Review-first product validation
          </h2>
          <p className="mt-3 max-w-4xl text-sm leading-7 text-gray-300">
            The Product Development & Validation Agent can summarize, score, and recommend follow-up opportunities.
