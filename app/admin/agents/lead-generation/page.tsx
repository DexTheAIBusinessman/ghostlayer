...
export const metadata = {
  title: "Lead Generation Agent | Ghostlayer Admin",
  description: "Read-only Ghostlayer lead generation agent for scan and lead follow-up review.",
};
...
async function fetchTableSummary(check: (typeof leadChecks)[number]): Promise<TableSummary> {
...
function buildRecommendations(summaries: TableSummary[]) {
  ...
  if (actions.length === 0) {
    actions.push("No recent lead-generation activity found in the last 14 days.");
  }
...
function AgentNightSkyBackground() {
  return (
    <div className="agentSky" aria-hidden="true">
      ...
      {[
        ...
        { left: "73%", top: "57%", size: 2, delay: "1.8s", duration: "5.9s" },
        { left: "78%", top: "11%", size: 2, delay: "0.2s", duration: "5.1s" },
        { left: "84%", top: "41%", size: 3, delay: "1.5s", duration: "5.6s" },
        { left: "88%", top: "76%", size: 2, delay: "2.6s", duration: "5.2s" },
        { left: "94%", top: "17%", size: 2, delay: "0.8s", duration: "5.5s" },
        { left: "97%", top: "61%", size: 2, delay: "2.3s", duration: "5.4s" },
      ].map((star, index) => (
        <span
          key={index}
          className="agentStar"
  ...
  return (
    <style>{`
      ...
      .agentMoon {
        ...
        border-radius: 9999px;
        background:
          radial-gradient(circle at 42% 38%, rgba(255, 255, 255, 0.20), transparent 0 3px),
          radial-gradient(circle at 52% 58%, rgba(255, 255, 255, 0.10), transparent 0 2px),
          radial-gradient(circle at 35% 62%, rgba(255, 255, 255, 0.08), transparent 0 2px),
          radial-gradient(circle at 50% 50%, rgba(255, 255, 255, 0.26), rgba(148, 163, 184, 0.12) 45%, rgba(30, 41, 59, 0.10) 70%, transparent 72%);
        box-shadow:
          0 0 90px rgba(147, 197, 253, 0.10),
          inset -60px -40px 90px rgba(15, 23, 42, 0.30),
          inset 35px 30px 80px rgba(255, 255, 255, 0.07);
        filter: blur(0.2px);
        opacity: 0.78;
      ...
      .agentFogB {
        ...
        animation: agentFogDriftTwo 48s ease-in-out infinite;
      }
...
export default async function LeadGenerationAgentPage() {
  const summaries = await Promise.all(leadChecks.map(fetchTableSummary));
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#05070b] px-6 py-10 text-white sm:px-8 lg:px-10">
      ...
      <section className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="mt-12 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-5 max-w-3xl text-sm leading-7 text-gray-300 sm:text-base">
              Read-only lead assistant for Ghostlayer. This agent reviews recent leads,
              workflow scans, CTA clicks, feedback, and client messages to identify
              follow-up opportunities for admin review.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <h2 className="mt-3 text-2xl font-black text-white">
                Recent lead-generation activity
              </h2>
              <p className="mt-3 max-w-3xl text-sm leading-7 text-gray-300">
                Showing recent lead-related activity from the last 14 days. This agent can
                summarize and recommend follow-up, but it does not contact leads or clients automatically.
              </p>
            </div>
        ...
        <section className="mt-8 rounded-[2rem] border border-cyan-300/20 bg-cyan-300/10 p-6 shadow-[0_24px_100px_rgba(0,0,0,0.25)] backdrop-blur-xl">
          ...
          <h2 className="mt-3 text-2xl font-black text-white">
            Leads and records worth reviewing first
          </h2>
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-2">
          {summaries.map((summary) => {
            const check = leadChecks.find((item) => item.table === summary.table);
        ...
        <section className="mt-8 rounded-[2rem] border border-yellow-300/20 bg-yellow-300/10 p-6 shadow-[0_24px_100px_rgba(0,0,0,0.25)] backdrop-blur-xl">
          ...
          <h2 className="mt-3 text-2xl font-black text-white">
            Review-first lead assistance
          </h2>
          <p className="mt-3 max-w-4xl text-sm leading-7 text-gray-300">
            The Lead Generation Agent can summarize, score, and recommend follow-up opportunities.
