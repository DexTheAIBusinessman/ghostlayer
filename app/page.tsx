...
export default function HomePage() {
  ...
  useEffect(() => {
    const interval = window.setInterval(() => {
      setWorkflowHealth((v) =>
        Math.max(78, Math.min(90, v + (Math.random() > 0.5 ? 1 : -1)))
      );
      setRiskScore((v) =>
        Math.max(60, Math.min(74, v + (Math.random() > 0.5 ? 1 : -1)))
      );
      setMonthlyLoss((v) =>
        Math.max(2800, Math.min(3800, v + Math.round((Math.random() - 0.5) * 140)))
      );
      setRecoveryOpportunity((v) =>
        Math.max(3600, Math.min(5000, v + Math.round((Math.random() - 0.5) * 180)))
      );
    }, 4200);
      ...
      <section id="results" className="relative z-10 px-4 py-5 sm:px-6 md:px-8 lg:px-10 xl:px-12">
        <div className="mx-auto max-w-7xl rounded-[30px] border border-white/8 bg-white/[0.03] p-5 shadow-[0_10px_40px_rgba(0,0,0,0.18)] backdrop-blur-sm sm:p-6">
          ...
          <div className="mt-5 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="metricCard metricBlue hoverCard metricInteractive p-5">
              <p className="metricLabel text-cyan-100">Risk Score</p>
              <p className="metricValue">
                <AnimatedNumber value={riskScore} suffix="/100" />
              </p>
